...
function Prompts() {
  ...
  const active = data?.versions.find((v) => v.id === data.prompt.active_version_id) ?? data?.versions[0];
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["prompt"] }); toast.success("Nova versão salva e ativada"); },
    onError: (e: Error) => toast.error(e.message),
  });
  const actMut = useMutation({
    mutationFn: (id: string) => activate({ data: { version_id: id } }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["prompt"] }); toast.success("Versão ativada"); },
  });
  ...
  return (
    <div className="mx-auto grid h-full w-full max-w-6xl grid-cols-1 gap-6 overflow-y-auto p-6 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="space-y-3">
        <div>
          <div className="font-mono-tabular text-[10px] uppercase tracking-widest text-primary">Timeline</div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {v.id === data.prompt.active_version_id && (
                    <Badge variant="outline" className="border-primary/40 text-primary text-[10px]">ativa</Badge>
                  )}
                </div>
                <span className="text-[10px] text-muted-foreground">{new Date(v.created_at).toLocaleDateString()}</span>
              </div>
              <div className="mt-1 line-clamp-2 text-[11px] text-muted-foreground">{v.content.slice(0, 120)}…</div>
              {v.id !== data.prompt.active_version_id && (
                <span
                  role="button"
                  onClick={(e) => { e.stopPropagation(); actMut.mutate(v.id); }}
      ...
      <section className="flex min-h-0 flex-col gap-3">
        <header>
          <div className="font-mono-tabular text-[10px] uppercase tracking-widest text-primary">Prompt Studio</div>
          <h1 className="font-display text-4xl">System Prompt do motor</h1>
          <p className="mt-1 text-sm text-muted-foreground">Edite as instruções que o Gym.AI segue em todos os discoveries. Salvar cria uma nova versão e ativa automaticamente.</p>
        </header>
        <div className="flex justify-end">
          <AttachTextFile
            label="Carregar de .txt / .md"
        ...
        />
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted-foreground font-mono-tabular">{draft.length} caracteres</span>
          <Button onClick={() => saveMut.mutate()} disabled={saveMut.isPending || draft === active?.content}>
